...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # load the motion file from a local path or the wandb registry
    registry_name = args_cli.registry_name
    if args_cli.motion_file is not None:
        env_cfg.commands.motion.motion_file = os.path.abspath(args_cli.motion_file)
        registry_name = f"local:{env_cfg.commands.motion.motion_file}"
        print(f"[INFO]: Using local motion file: {env_cfg.commands.motion.motion_file}")
    else:
        if registry_name is None:
            raise ValueError("Provide either --motion_file or --registry_name.")
